...
impl From<LexError> for Error {
    fn from(e: Annot<LexErrorKind>) -> Self {
        Error::Lexer(e)
    }
}
...
macro_rules! lex_symbol {
    ($(($name:ident, $sym:expr, $token:expr)),*) => {
        $(
            fn $name(input: &[u8], start: usize) -> Result<(Token, usize), LexError> {
                consume_byte(input, start, $sym).map(|(_, end)| ($token(Loc(start, end)), end))
            }
        )*
    };
}
...
lex_symbol!(
    (lex_plus, b'+', Token::plus),
    (lex_minus, b'-', Token::minus),
    (lex_asterisk, b'*', Token::asterisk),
    (lex_slash, b'/', Token::slash),
    (lex_lparen, b'(', Token::lparen),
    (lex_rparen, b')', Token::rparen)
);
...
//                    P::UnexpectedToken(Token { loc, .. }) |
                    P::NotExpression(Token { loc, .. }) | P::NotOperator(Token { loc, .. }) | P::UnclosedOpenParen(Token { loc, .. }) =>
                        loc.clone(),
                    P::RedundantExpression(Token { loc, .. }) =>
                        Loc(loc.0, input.len()),
                    P::Eof =>
                        Loc(input.len(), input.len() + 1),
                };
                (e, loc)
            }
        };
...
fn main() {
    ...
    use std::mem;
    println!("{}", mem::size_of::<SomeEnum>()); // 32
    println!("{}", mem::size_of::<&SomeEnum>()); // 8
...
fn parse_expr3<Tokens>(toknes: &mut Peekable<Tokens>) -> Result<Ast, ParseError>
    where Tokens: Iterator<Item=Token>
{
    ...
    parse_left_binop(toknes, parse_expr2, parse_expr3_op)
}

fn parse_expr2<Tokens>(toknes: &mut Peekable<Tokens>) -> Result<Ast, ParseError>
    where Tokens: Iterator<Item=Token>
{
    ...
    parse_left_binop(toknes, parse_expr1, parse_expr2_op)
}
