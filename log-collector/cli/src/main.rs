use clap::{App, AppSettings, Arg, SubCommand, arg_enum};
use std::io;
use reqwest::Client;
...
fn main() {
    ...
    let server = matches.value_of("SERVER").unwrap_or("localhost:3000").into();
    ...
    match matches.subcommand() {
        ("get", sub_match) => {
            ...
            match format {
                ...
                Format::Json => {
                    do_get_json(&api_client);
                },
            }
        }
        ("post", _) => {
            do_post_csv(&api_client);
        },
        _ => unreachable!(),
    };
}
